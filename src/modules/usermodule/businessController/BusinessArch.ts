...
class BussnesArchivo{
    ...
    public async readArch(query: any, skip: number, limit: number): Promise<Array<IArchivo>>;

    public async readArch(params1?: string | any, params2?: number, params3?: number): Promise<Array<IArchivo> | IArchivo> {
        if (params1 && typeof params1 == "string") {
            ...
            let listArch: Array<IArchivo> = await ArchivoModel.find(params1).skip(skip).limit(limit);
            return listArch;
        } 
        else {
