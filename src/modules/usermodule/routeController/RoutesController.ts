...
class RoutesController {
 ...
 //Imprimir Desembolso 
  ...
  public async getSeguiId(request: Request, response: Response) {
    var segui: BussinesSegui = new BussinesSegui();
    let id: string = request.params.id;
    let res = await segui.getSeguiId(id);
    response.status(200).json(res);
  }
  ...
  public async updateSeguiOfi(request: Request, response: Response) {
    ...
    var params = request.body;
    let ofi = { destino: "INSTANCIA TECNICA DE SALUD DEL G.A.M.B.", estado: "FILE OFICINA" };
